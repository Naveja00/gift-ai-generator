 codex/build-ai-powered-giftcurator-web-app-8uy8mo
...
const buildPrompt = (payload) => `You are "The Curatorial Expert," a witty, empathetic AI gift curator.
Based on the user's inputs, generate a curated list of 5 thoughtful gifts.
Return strict JSON only with keys: title, summary_for_user, gift_ideas.

gift_ideas must be an array of exactly 5 objects each with:
- rank (1-5)
- gift_name
- reasoning
- style_match
...
- Gift Tone: ${payload.tone === 'No Preference' ? 'No Preference / choose best fit naturally' : payload.tone}

Rules:
- Be specific, practical, and varied.
- No markdown fences. JSON only.`;

const fallbackIdeas = (payload) => ({
  title: `${payload.occasion} Gifts for ${payload.recipient}`,
  summary_for_user: `Here are thoughtful gift ideas for ${payload.recipient} based on ${payload.interests} within ${payload.budget}.`,
  gift_ideas: Array.from({ length: 5 }).map((_, i) => ({
    rank: i + 1,
    gift_name: `${payload.interests || 'Personalized'} Gift Idea ${i + 1}`,
    reasoning: `Fits ${payload.recipient}'s interests and the ${payload.occasion} occasion.`,
    style_match: payload.tone === 'No Preference' ? 'Balanced and broadly appealing.' : `Matches a ${payload.tone.toLowerCase()} vibe.`,
    amazon_search_query: `${payload.interests} gift ${payload.budget}`
  }))
});

const invokeGiftAI = async (payload) => {
  const res = await fetch(OLLAMA_URL, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ model: OLLAMA_MODEL, prompt: buildPrompt(payload), stream: false, format: 'json' })
  });

  if (!res.ok) throw new Error('Ollama request failed');
  const data = await res.json();
  const parsed = JSON.parse(data.response);
  if (!parsed?.gift_ideas || parsed.gift_ideas.length !== 5) throw new Error('Invalid AI JSON shape');
  return parsed;
};
 ...
 codex/build-ai-powered-giftcurator-web-app-8uy8mo
  const mutation = useMutation({
    mutationFn: async (payload) => {
      try {
        return await invokeGiftAI(payload);
      } catch {
        return fallbackIdeas(payload);
      }
    },
    onSuccess: (data) => setResult({ ...form, ...data, gifts: data.gift_ideas.map((g) => ({ ...g, link: buildAmazonLink(g.amazon_search_query) })) })
  });
